...
std::string executePHPFile(const std::string &filePath)
{
    std::string command = "php " + filePath + " 2>&1"; // Execute PHP file
    std::array<char, 128> buffer;
    std::string result;
    FILE *pipe = popen(command.c_str(), "r");
    if (!pipe)
    {
        return "Failed to execute PHP file.";
    }
...
std::string findDefaultIndex(const std::string &webDir, const std::vector<std::string> &defaultIndexes)
{
    for (const auto &indexFile : defaultIndexes)
    {
        std::string fullPath = webDir + "/" + indexFile;
        std::ifstream file(fullPath);
        if (file.good())
        {
            return indexFile; // Return the first index file that exists
        }
    }
    return ""; // Return an empty string if none of the index files are found
}
...
int main()
{
    ...
    std::vector<std::string> defaultIndexes = split(config["default_index"], ' ');
    std::string error404 = config["error_404"];
    ...
    while (true)
    {
        ...
        // If no specific file is requested, serve the default index file
        if (requestedFile == "/")
        {
            std::string defaultFile = findDefaultIndex(webDir, defaultIndexes);
            if (defaultFile.empty())
            {
                requestedFile = "/" + error404; // No index file found, use the 404 error file
            }
            else
            {
        ...
        if (filePath.substr(filePath.find_last_of(".")) == ".php")
        {
            // Execute the PHP file and get the output
            std::string phpOutput = executePHPFile(filePath);
            response = "HTTP/1.1 200 OK\nContent-Type: text/html\nContent-Length: " + std::to_string(phpOutput.size()) + "\n\n" + phpOutput;
        }
        else
        {
            ...
            else
            {
                ...
                std::string errorContent = readFile(webDir + "/" + error404);
                if (errorContent.empty())
                {
                    errorContent = "<html><body><h1>404 Not Found</h1></body></html>";
                }
                response = "HTTP/1.1 404 Not Found\nContent-Type: text/html\nContent-Length: " + std::to_string(errorContent.size()) + "\n\n" + errorContent;
            }
        }
